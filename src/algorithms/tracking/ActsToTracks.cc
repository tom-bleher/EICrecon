...
void ActsToTracks::process(const Input& input, const Output& output) const {
  ...
  for (const auto& track : acts_track_container) {
      ...
      if (state.hasUncalibratedSourceLink()) {
        ...
        if (typeFlags.test(Acts::TrackStateFlag::HoleFlag)) {
          ...
          if (typeFlags.test(Acts::TrackStateFlag::OutlierFlag)) {
            ...
            const double total_edep = sum_weights(measurement_edep);
            const auto& measurement_weights = total_edep > 0.0 ? measurement_edep : measurement_counts;
            const double measurement_total = sum_weights(measurement_weights);
            if (measurement_total > 0.0) {
              for (const auto& [mc_particle, contribution] : measurement_weights) {
                mcparticle_weight_by_measurement[mc_particle] += contribution / measurement_total;
              }
            }
          }
        }
    ...
    double total_weight = std::accumulate(
        mcparticle_weight_by_measurement.begin(), mcparticle_weight_by_measurement.end(), 0,
