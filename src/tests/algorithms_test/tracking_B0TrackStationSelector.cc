...
TEST_CASE("B0 station mapping preserves four-station layouts", "[tracking][b0stations]") {
  ...
  const auto disks = makeB0SurfaceStationMap(
      {{9, 5902.}, {8, 6172.}, {7, 6442.}, {6, 6712.}}, 50.);
  ...
  std::vector<std::pair<std::uint64_t, double>> faces{
      {91, 6000.}, {23, 6007.}, {5, 6276.}, {1, 6283.},
...
TEST_CASE("B0 station mapping validates surface identity", "[tracking][b0stations]") {
  ...
  CHECK(expected == makeB0SurfaceStationMap(
                        {{23, 6007.}, {91, 6000.}, {23, 6007.}}, 50.));
  ...
  CHECK_THROWS_AS(makeB0SurfaceStationMap({{91, 6000.}, {91, 6007.}}, 50.),
                  std::invalid_argument);
  CHECK_THROWS_AS(makeB0SurfaceStationMap({{91, 6007.}, {91, 6000.}}, 50.),
                  std::invalid_argument);
}
...
TEST_CASE("B0 station mapping retains single-linkage gap semantics", "[tracking][b0stations]") {
  ...
  const auto mapped = makeB0SurfaceStationMap(
      {{4, 100.01}, {3, 50.}, {2, 0.}, {1, -50.}}, 50.);
